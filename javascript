...
Каждый раз, когда происходит 'событие'(пользователи нажимают кнопки,
отслеживаются перемещения мыши, поступают данные, меняются размеры
окон, срабатывают таймеры и т. д.), вашему коду предоставляется возможность обработать его.

'Обработчики' пишутся для обработки событий. 
Как правило, обработчик представляет собой небольшой фрагмент кода, который должен выпол-
няться при возникновении события.
В программном коде обработчик представляет собой 'обычную функцию'. 
Когда происходит событие, вызывается функция-обработчик.

Чтобы ваш обработчик вызывался при возникновении события, его необ-
ходимо сначала 'зарегистрировать'.

Событие загрузки страницы инициируется тогда, когда
браузер полностью загрузил и отобразил все содержимое страницы (и построил модель
DOM, представляющую страницу).
	...
	Напоминаем: функция init вызы-
	вается function после завершения загрузки страницы.
	Но функция showAnswer будет вызвана позднее, когда
	пользователь щелкнет на изображении. Итак, эти два
	обработчика событий вызываются в разное время.
	Также следует помнить правила видимости имен. В функ-
	ции init объект, полученный при вызове getElementById ,
	присваивается локальной переменной image ; это означа-
	ет, что при завершении функции переменная выходит из
	области действия и уничтожается. Позднее, при вызове
	функции showAnswer , нам приходится заново получать
	объект изображения из DOM. Конечно, объект можно
	было сохранить в глобальной переменной, но слишком
	частое использование глобальных переменных ведет
	к появлению запутанного кода и повышает вероятность
	ошибок.

	Назначение свойства src изображения — то же, что назна-
	чение атрибута src вызовом setAttribute? 

	В данном случае — да. При получении элемента HTML из DOM
	вызовом getElementById вы получаете объект element с набором
	методов и свойств. У всех объектов element имеется свойство id,
	содержащее идентификатор элемента HTML (если он был назначен
	в HTML). Объект element изображения также содержит свойство src,
	которому назначается графический файл, указанный в атрибуте
	src элемента <img>.
	...
	Выходит, обработчик вызывается внутри другого обра-
	ботчика?

	 Не совсем. Обработчик onload содержит код, вызываемый
	при завершении загрузки страницы. При его вызове мы назначаем
	обработчик свойству onclick элемента изображения, но вызван он
	будет только тогда, когда пользователь щелкнет на изображе-
	нии — теоретически это может произойти значительно позднее
	завершения загрузки страницы. Получается, что два обработчика
	вызываются в разное время.
	...
	function init() {
		...
		Вы сказали, что метод
		getElementsByTagName возвращает
		список. Имеется в виду массив?

		Метод возвращает объект, с которым
		можно работать как с массивом, но на са-
		мом деле это объект типа NodeList. Объект
		NodeList представляет коллекцию узлов
		(технический термин для объектов элемен-
		тов в дереве DOM). Чтобы выполнить пере-
		бор коллекции, следует получить ее длину
		из свойства length, а затем последовательно
		обратиться к каждому элементу NodeList
		по индексу в квадратных скобках (так же,
		как это делается с массивами). Впрочем,
		на этом сходство между NodeList и масси-
		вом завершается, поэтому в остальном при
		работе с объектом NodeList необходимо
		действовать осторожно. Обычно это все, что
		необходимо знать о NodeList, если только
		вы не собираетесь добавлять и удалять
		элементы из DOM.

		Функции-обработчики получают
		аргументы?

		Хороший вопрос... и очень своевремен-
		ный. Да, получают, и мы как раз собираемся
		рассмотреть объект события, передавае-
		мый некоторым обработчикам.



	При каждом вызове обработчика события
	щелчка передается объект события. Мы можем воспользоваться
	этим объектом для получения подробной информации о событии.

	При вызове обработчик события щелчка получает объект события — бо-
	лее того, этот объект передается для большинства событий, связанных
	с моделью DOM. Объект события содержит общую информацию о собы-
	тии: какой элемент породил это событие, в какое время оно произошло
	и т. д. Кроме того, передается информация, связанная с конкретным
	событием: например, для щелчка мышью вы получите координаты
	точки щелчка.
...
обратиться следующим образом:
	...
	Обработчику события onload тоже передается
	объект события?
	О:
	Да, передается. В нем содержится информация
	об источнике (объект window), времени возникновения
	события и типе события (“load”). Можно с уверенностью
	сказать, что объект события в обработчиках onload ис-
	пользуется очень редко, потому что для этого события
	хранящаяся в нем информация не столь актуальна. Иногда
	объект события полезен, иногда нет — все зависит от типа
	события. Если вы не уверены в том, какую информацию
	содержит объект конкретного типа события, обращайтесь
	к справочнику по JavaScript.

Браузер: Даже если обработчика нет, работы все
равно хватает. Кто-то должен обнаружить событие,
интерпретировать его и проверить, назначен ли
для него обработчик. Если обработчик есть, нужно
передать ему управление.
Браузер: Да, за короткий промежуток времени
может случиться множество событий. Иногда собы-
тия происходят слишком быстро, чтобы работать
в реальном времени. Поэтому я ставлю их в очередь
по мере поступления, а затем прохожу по очереди
и запускаю обработчики там, где это необходимо.
Браузер: Допустим, вы пишете обработчик, тре-
бующий большого объема вычислений, — то есть
обработчик, на выполнение которого потребуется
много времени. Пока ваш обработчик усердно тру-
дится, я сижу и ожидаю, когда он завершится. Только
тогда я смогу продолжить обработку очереди.
Браузер: Много. Есть события, относящиеся к пе-
редаче данных, события DOM, связанные со стра-
ницей, и много других. События DOM, например,
генерируют объекты событий, содержащие более
подробную информацию о событии. В событии
щелчка мышью есть информация о том, в какой
точке он был сделан, а в событии нажатия клави-
ши — о нажатой клавише и т. д.
...
если он имеется.
Браузер обрабатывает события последовательно, поэтому обра-
ботчики должны быть по возможности короткими и эффективны-
ми. В противном случае очередь событий может переполниться,
и браузер столкнется с необходимостью обработать их все. Что это
означает для вас? Что интерфейс начнет «тормозить» и с большими
задержками реагировать на действия пользователя.


Событие перемещения мыши сообщает обработчику о том,что указатель мыши переместился над некоторым элементом.
Обработчик назначается свойству onmousemove элемента. При вызове обработчику передается объект события, который предоставляет следующие свойства:
...
Для всех событий, которые вы видели, обработчик всегда назначался неко-
торому свойству — onload , onmouseover , onclick и т. д. Но не все события
работают так, например для хронометражных событий программист вместо
этого вызывает функцию setTimeout , передавая ей свой обработчик.
	...
	При загрузке страницы мы
	выполняем две операции: на-
	значаем обработчик с именем
	timerHandler и вызываем функ-
	цию setTimeout для создания
	события таймера, которое будет
	сгенерировано через 5000 мил-
	лисекунд. Когда сработает собы-
	тие таймера, обработчик будет
	выполнен.
	Пока таймер отсчитывает
	миллисекунды, браузер про-
	должает работать как обычно.
	Когда отсчет времени доходит до нуля,
	...
	Tock
		...
		Можно ли остановить отсчет setInterval?
		Можно. При вызове setInterval вы получаете объект таймера,
		если его передать функции clearInterval, можно остановить таймер.

		Формально мы могли бы использовать запись
		window.setTimeout, но посколько объект window object считается
		глобльным, мы можем опустить имя объекта и использовать сокра-
		щенную форму setTimeout, которая часто встречается на практике.

		При вызове setTimeout вы создаете таймер и связываете
		с ним обработчик. Таймеров можно создать сколько угодно. Браузер
		следит за тем, какой обработчик связан с тем или иным таймером.


		Можно ли опустить window в синтаксисе window.onload?
		Можно, но обычно программисты так не делают, потому что
		onload является достаточно распространенным именем свойства
		(у других элементов тоже есть свойство onload), и его использование
		без уточнения может создать путаницу.

		Как насчет объекта события? Поче-
		му setTimeout не передает его обработ-
		чику события?
		О:
		Объекты событий в основном использу-
		ются с обработчиками событий DOM. Функ-
		ция setTimeout не передает обработчику
		объекты событий, потому эти события не
		происходят ни с каким конкретным эле-
		ментом.

		Итак, существуют события DOM,
		события таймеров... Какие еще?
		О:
		Большинство событий относятся к ка-
		тегории событий DOM (например, щелчок
		на элементе) или событий таймеров (созда-
		ются вызовами setTmeout или setInterval).
		Существуют события конкретных API,
		например JavaScript API, относящиеся
		к Geolocation, LocalStorage, Web Workers
		и т. д. (обращайтесь к книге Изучаем про-
		граммирование на HTML5 ). Наконец, су-
		ществует целая категория событий, отно-
		сящихся к вводу/выводу, например запрос
		данных от веб-служб с использованием
		XmlHttpRequest (также обращайтесь к Изу­
		чаем программирование на HTML5 ) или
		Web Sockets.


//////
///////////////////
ольшая часть кода JavaScript пишется для обработки
событий.
  Существует много разных видов событий, на которые может
реагировать ваш код.
  Чтобы отреагировать на событие, следует написать функ-
цию — обработчик события и зарегистрировать ее.
Например, чтобы зарегистрировать обработчик для события
щелчка, следует присвоить функцию-обработчик свойству
onclick элемента.
  Вы не обязаны обрабатывать какие-либо события в своих
...
  Код, написанный для обработки событий, отличается от кода,
который выполняется от начала до конца, а потом завер-
шается. Обработчики событий могут выполняться в любое
время и в любом порядке: они выполняются асинхронно.
  События, происходящие с элементами DOM (события DOM),
передаются в обработчик событий.


  Объект события содержит свойства с дополнительной
информацией о событии, включая тип (например, “click” или
“load”) и источник (объект, с которым произошло событие).
  В старых версиях IE (IE 8 и ранее) используется модель
события, отличающаяся от других браузеров.
  Многие события могут происходить с большой частотой. Ког-
...
последовательно выполнять обработчики всех событий.
  Если обработка события требует значительного объема
...
  Функции setTimeout и setInterval используются для
генерирования событий таймера по истечении заданного
интервала времени.
  Метод getElementsByTagName возвращает нуль, один
или несколько объектов элементов, объединенных в объект
NodeList (аналог массива с возможностью перебора).

/////////////////////////////

Объявление создает функцию с именем (в данном случае quack ),
которое может использоваться для ссылок и вызова функции.

Такое использование ключевого слова function — внутри
команды, как в команде присваивания, — называется
функциональным выражением. Обратите внимание: в отли-
чие от объявления, эта функция не имеет имени.

Ссылка на функцию — это... Да в общ
ем
именно то, что следует из назван
ия,  —
ссылка, которая указывает на фун
кцию.
Ссылки могут использоваться для
вы­
зова функции. Кроме того, их мож
но
присваивать переменным, сохран
ять
в  объектах, передавать и возвра
щать
из функций (как и ссылки на объ
екты).

Я знаю, что это
вызов функции, потому что мы
используем имя переменной fly,
за которым следуют круглые скобки.
Вспомните, что переменной fly
хранится ссылка на функцию,
которая была сохранена ранее.

Объявления функций обрабатываются
раньше остального кода.
  Функциональные выражения обрабатыва­
ются позднее, вместе с остальным кодом.
  Объявление функции не возвращает
ссылку на функцию; вместо этого оно
создает переменную с именем функции
и присваивает ей новую функцию.
  Функциональное выражение возвращает
ссылку на новую функцию, созданную
выражением.
  Ссылки на функции могут храниться в пе­
ременных.

Объявления функций являются команда­
ми; функциональные выражения исполь­
зуются в командах.
  Процесс вызова функции, созданной по
объявлению, не отличается от процесса
вызова функции, созданной по выраже­
нию.
  Объявления — традиционный способ
создания функций.
  Используйте объявления функций там,
где это возможно, потому что они обраба­
тываются на более ранней стадии.


Мы видели выражения вида 3+4
и Math.random() * 6, но как функция может
быть выражением?
О:
Выражением может быть все, что при
вычислении дает некоторое значение. 3+4
дает результат 7, Math.random() * 6 дает
случайное число, а функциональное выра-
жение дает ссылку на функцию.


Но объявление функции не являет-
ся выражением?
Нет, объявление функции является
командой. Считайте, что здесь выполняется
скрытое присваивание, которое связывает
ссылку на функцию с переменной за вас.
Функциональное выражение не присваивает
ссылку на функцию ничему; вам приходится
делать это самостоятельно.


Хорошо, в переменной может хра-
ниться ссылка на функцию. Но на что
конкретно эта ссылка указывает? На ка-
кой-то код в теле функции?
О:
Это хорошая точка зрения на ссылки,
...
ные» функции содержат нечто большее, чем
просто код из своего тела.


с объявлением функции, и с функциональным
выражением вы получаете функцию. Но между
ними существует ряд важных отличий. Прежде
всего, с объявлением функция создается и подго-
тавливается до обработки остального кода. С функ-
циональным выражением функция создается при
выполнении кода, на стадии выполнения.

Другое отличие связано с именами функций —
с объявлением имя функции используется для
создания переменной, содержащей ссылку на
функцию. Кроме того, при использовании функ-
ционального выражения имя функции обычно не
указывается, а функция либо присваивается пере-
менной в коде, либо функциональное выражение
используется иным образом.

Обычно мы представляем себе функции как нечто такое, что можно вызвать, — однако функ-
ции также можно рассматривать и как значения. Это значение в действительности является
ссылкой на функцию, и как вы уже видели, независимо от способа определения функции
(через объявление или функциональное выражение) вы получаете ссылку на эту функцию
...
И хотя переменная quack была созда-
на по объявлению функции, хранящееся
в quack значение тоже представляет
собой ссылку на переменную, поэтому
мы можем присвоить его переменной
superQuack и вызвать функцию.

Теперь вы знаете, что функции в JavaScript являются значениями —
значениями, которые могут присваиваться переменным. И вы знаете,
что со значениями других типов (таких, как числа, булевские значения,
строки и даже объекты) можно проделывать много полезных операций,
например передавать их функциям, возвращать из функций и даже
сохранять в объектах или массивах.

Присваивать функции переменным.
Передавать функции функциям.
Возвращать функции из функций


Что именно передается при передаче функции другой
функции?
Передается ссылка на функцию. Считайте, что ссылка — это
своего рода указатель на внутреннее представление функции. Сама
ссылка может храниться в переменной, ее можно присваивать
другим переменным и передавать в аргументах функций. Если
после ссылки на функцию стоят круглые скобки, то это приводит
к вызову функции.
...
c.504
Прежде всего то, что вы можете размещать объявления функций в любой точке
...
Ничто не мешает нам определять функции внутри других функций; это означает, что объявления функ-
ций или функциональные выражения можно использовать только внутри этих функций. Как работает
вложение? Очень просто: различия между функцией, определяемой на верхнем уровне кода, и функцией,
определяемой внутри другой функции, сводятся к области действия имен. Другими словами, размещение
функции внутри другой функции влияет на видимость этой функции внутри вашего кода.
Функции, определяемые на верхнем уровне кода, обладают
глобальной областью действия, тогда как функции, определяе-
мые внутри других функций, обладают локальной видимостью.

Для функций, определяемых внутри функций, по возможности
обращения к функции действуют те же правила, что и на верх-
нем уровне. Другими словами, внутри функции при определе-
нии вложенной функции посредством объявления эта вложенная
функция определена в любой точке тела функции. С другой
стороны, если при создании вложенной функции используется
функциональное выражение, то эта вложенная функция опреде-
лена только после обработки функционального выражения.

При передаче функционального выра-
жения другой функции эта функция сохраня-
ется в параметре, а затем рассматривается
как локальная переменная в той функции,
которой она передается. Верно?
О:
Совершенно верно. При передаче функции
в аргументе передаваемая ссылка копируется
в переменную-параметр вызываемой функции.
И параметр, содержащий ссылку на функцию,
является локальной переменной (как, впрочем,
и любой другой параметр).
...
1 Сначала мы обнаруживаем локальную переменную
с именем justAVar. Этой переменной присваивается
строка “Just an every day LOCAL”.

2 Ранее об этом не упоминалось, но все ло-
кальные переменные хранятся в окружении
(environment).
		var justAVar = "Just an every day LOCAL";
		function inner() {
			return justAVar;
		}
Это окружение. В нем хранятся все пе-
ременные, определенные в локальной
области действия.

3 Затем создается функция
с именем inner.

4 Наконец, при возвращении функции
возвращается не только сама функция,
но и присоединенное к ней окружение.

С каждой функцией связывается
окружение, которое содержит
локальные переменные из внешней
области действия.


1
Сначала вызывается whereAreYou. Мы уже знаем, что она возвращает ссылку на функцию,
поэтому создаем переменную innerFunction и присваиваем ей полученную функцию. Пом-
ните, что ссылка на функцию связана со своим окружением?

var innerFunction = whereAreYou();

После этой команды появляется
переменная innerFunction, кото-
рая содержит ссылку на функцию
(а также ее окружение), возвра-
щаемую из whereAreYou.

2 Затем вызывается innerFunction. Для этого мы обрабатываем код тела функции,
причем делаем это в контексте ее окружения:
3 Наконец, результат функции присваивается переменной result
и выводится на консоль.
innerFunction возвращает значение "Just an every day LOCAL" ,которое берется из окружения.

Как работает окружение для функций, вложенных на не-
сколько уровней?
Мы использовали для объяснения упрощенную модель окру-
жения, но вы можете считать, что каждая вложенная функция имеет
собственное окружение с собственными переменными. Далее
остается лишь создать цепочку окружений для всех вложенных
функций, от внутренних к внешним.
Итак, поиск переменной начинается с ближайшего окружения, а за-
тем проходит по цепочке, пока переменная не будет найдена. А если
найти ее не удалось — поиск переходит к глобальному окружению.

Переменные-параметры тоже включаются в окружение?

Да. Как было сказано ранее, с параметрами в функциях мож-
но работать как с локальными переменными, поэтому они тоже
включаются в окружение.

Обязательно ли во всех подробностях понимать, как
работает окружение?

Нет. Нужно понимать правила лексической области действия
для переменных — мы о них уже рассказали. А еще вы знаете,
что функция, возвращаемая из функции, несет с собой все свое
окружение.

Помните, что функции JavaScript всегда выполняются
в том же окружении, в котором они определяются.
Чтобы внутри функции узнать, откуда появилась
переменная, проведите поиск в ее внешних функциях,
от наибольшего уровня вложенности к наименьшему.
................
...
Вероятно, вы это уже поняли из предыдущего упражне-
ния, но давайте повторим еще раз: в коде тела функции
обычно присутствуют локальные переменные (включая все
параметры функции), а также могут присутствовать пере-
менные, не определяемые локально, — такие переменные
называются свободными. Термин «свободный» появился
потому, что в теле функции свободные переменные не
связываются ни с какими значениями (иначе говоря,
они объявляются где-то за пределами функции). Теперь в
окружении, содержащем значение для каждой свободной
переменной, функция становится замкнутой. А если взять
функцию вместе с ее окружением, получится замыкание.

Если переменная в теле моей
функции не определяется локально
и не является глобальной, значит,
она получена из функции, в которую
вложена моя функция, и доступна
в моем окружении.

Свободной называется переменная, которая не объявляется в локальной области
действия.

Замыкание возникает тогда, когда функция,
содержащая свободные переменные, объединяется
с окружением, которое предоставляет привязки
для всех свободных переменных.
==================== Ex.===================
	...
	var pass1 = makePassword(343);
	var pass2 = makePassword(123);

	console.log(pass1(545));             //false
	console.log(pass2(123));          // true
.............................
	...
	console.log(num2(2));
.................
1 Мы вызываем функцию makeCounter, которая создает
функцию counter и возвращает ее вместе с окружением,
содержащим свободную переменную count. Другими
словами, она создает замыкание. Функция, возвращен-
ная из makeCounter, сохраняется в doCount.

2 Вызываем функцию doCount. Это приводит к выпол-
нению тела функции counter.

3 Обнаружив переменную count, мы ищем ее в окруже-
нии и получаем ее значение. Переменная count увели-
чивается, новое значение сохраняется в окружении,
после чего функция возвращает это новое значение
в точку вызова doCount.
...
4 Шаги 2 и 3 повторяются при каждом вызове doCount.
	...
	function makeCounter() {
		...
		return counter;                 Это замыкание
	}

	var doCount = makeCounter();

При вызове makeCounter
мы получаем замыкание:
функцию с окружением.

	console.log(doCount());
	console.log(doCount());
	console.log(doCount());

При вызове doCount (переменной, содержащей ссылку на функцию
counter) значение count берется из переменной count, находящейся
в окружении замыкания. Для внешнего мира (то есть глобальной
области действия) переменная count остается невидимой. Тем не ме-
нее мы можем использовать ее при каждом вызове doCount. К count
невозможно обратиться иначе, кроме как при вызове doCount.
.................................
	function makeCounter() {
		var count = 0;
		function increment() {
...
Возвращение функции из функции — не единственный способ создания
замыканий. Они создаются везде, где появляется ссылка на функцию со
свободными переменными, и эта функция выполняется вне контекста,
в котором она была создана.
Замыкания также могут создаваться передачей функций при вызове функ-
ций. Передаваемая функция выполняется в контексте, отличном от кон-
текста ее создания.
...
doneMessage , передается функции setTimeout . Как вам известно, при этом
функциональное выражение обрабатывается для получения ссылки на
функцию, которая затем передается setTimeout . Метод setTimeout сохра-
няет функцию (а вернее, функцию вместе с окружением — иначе говоря,
замыкание), после чего через 1000 миллисекунд вызывает ее.
И снова функция, передаваемая setTimeout , представляет собой замыка-
ние, потому что вместе с ней передается окружение, связывающее свобод-
ную переменную doneMessage со строкой “Cookies are done!”.

Замыкание содержит непосредственное окружение, а не его копию
Многие разработчики при изучении замыканий ошибочно полагают, что окружение в замыкании долж-
но содержать копию всех переменных и их значений. Это не так. На самом деле окружение обращается
к «живым» переменным, используемым в вашем коде, так что если значение будет изменено за пределами
